...
class CallBack(object):
    def __init__(self, tag, sensor_type, sensor, data_provider, period_frames=1):
        self._tag = tag
        self._data_provider = data_provider

        self._data_provider.register_sensor(tag, sensor_type, sensor, period_frames)
...
class SensorInterface(object):
    ...
    def register_sensor(self, tag, sensor_type, sensor, period_frames=1):
        if tag in self._sensors_objects:
            raise SensorConfigurationInvalid("Duplicated sensor tag [{}]".format(tag))

        self._sensors_objects[tag] = sensor
        self._sensor_periods[tag] = period_frames
    ...
    def get_data(self, frame):
        ...
        try:
            ...
            start = time.time()
            arrivals = []
            # Low-rate sensors (period > 1) capture on the same frames as every
            # other sensor of the same period, but which frames those are is only
            # observable from the data itself. A period group is awaited once one
            # of its sensors has delivered data for this frame; blocking on a
            # predicted frame the group does not capture cannot be recovered, as
            ...
            while True:
                ...
                tag, data_frame, data = self._data_buffers.get(True, self._queue_timeout)
                if data_frame != frame:
                    continue
                arrivals.append((tag, time.time() - start))
                due_periods.add(self._sensor_periods[tag])
