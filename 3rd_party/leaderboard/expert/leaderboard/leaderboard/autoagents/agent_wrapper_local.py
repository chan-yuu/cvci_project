...
QUALIFIER_SENSORS_LIMITS = {
    'sensor.camera.rgb': 100,
    'sensor.camera.instance_segmentation': 100,
    'sensor.lidar.ray_cast': 100,
    'sensor.other.radar': 100,
    'sensor.other.gnss': 100,
    'sensor.other.imu': 100,
    'sensor.opendrive_map': 100,
    'sensor.speedometer': 100,
    'sensor.camera.depth': 100, # for data generation
    'sensor.camera.semantic_segmentation': 100, # for data generation
    'sensor.lidar.ray_cast_semantic': 1,
    'sensor.lidar.ray_cast_dense': 1,
}
SENSORS_LIMITS = {
    'sensor.camera.rgb': 100,
    'sensor.camera.instance_segmentation': 100,
...
class AgentWrapper(object):
    ...
    def _preprocess_sensor_spec(self, sensor_spec):
        ...
        elif type_ == 'sensor.lidar.ray_cast_dense':
            attributes['range'] = str(85)
            attributes['rotation_frequency'] = str(20)
            attributes['channels'] = str(64)
            attributes['upper_fov'] = str(10)
        ...
        # Only forward sensor_tick for low-rate sensors. Values at or below the
        # world delta mean "every tick", which is the attribute's default; setting
        # them explicitly risks skipped frames from float comparisons against the
        # accumulated game time.
        if type_ not in ('sensor.opendrive_map', 'sensor.speedometer'):
            delta_time = CarlaDataProvider.get_world().get_settings().fixed_delta_seconds
            if float(sensor_spec.get('sensor_tick', 0.0)) > delta_time:
    ...
    def setup_sensors(self, vehicle):
        ...
        for sensor_spec in self._agent.sensors():
            ...
            # These are the sensors spawned on the carla world
                ...
                type_ = type_.replace("sensor.lidar.ray_cast_dense", "sensor.lidar.ray_cast")
            ...
            # setup callback
            period_frames = max(1, math.ceil(float(attributes.get('sensor_tick', 0.0)) / delta_time))
            sensor.listen(CallBack(id_, type_, sensor, self._agent.sensor_interface, period_frames))
