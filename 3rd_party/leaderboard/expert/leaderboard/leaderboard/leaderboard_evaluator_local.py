...
from leaderboard.utils.statistics_manager_local import (
    FAILURE_MESSAGES,
    StatisticsManager,
)
...
class LeaderboardEvaluator(object):
    ...
    def _setup_simulation(self, args):
        ...
        # Honor --traffic-manager-port so several clients on one node do not
        # race find_free_port() (that helper also uses SO_REUSEADDR and can
        # return a port CARLA already owns). Fall back only if unset.
        traffic_manager_port = getattr(args, "traffic_manager_port", None)
        if not traffic_manager_port:
...
def main():
    ...
    parser.add_argument(
        '--traffic-manager-port', default=8000, type=int,
        help='Port to use for the TrafficManager (default: 8000)',
    )
    parser.add_argument(
        '--traffic-manager-seed', default=100, type=int,
    ...
    parser.add_argument(
        '--timeout', default=300.0, type=float,
    ...
    parser.add_argument(
        '--routes-subset', default='', type=str,
    ...
    parser.add_argument(
        "--agent-config", type=str,
        help="Path to Agent's configuration file", default="",
    )

    parser.add_argument(
        "--track", type=str, default='SENSORS',
        help="Participation track: SENSORS, MAP",
    )
    parser.add_argument(
        '--resume', type=int, default=False,
        help='Resume execution from last checkpoint?',
    )
    parser.add_argument(
        "--checkpoint", type=str, default='./simulation_results.json',
        help="Path to checkpoint used for saving statistics and resuming",
    )
    parser.add_argument(
        "--debug-checkpoint", type=str, default='./live_results.txt',
