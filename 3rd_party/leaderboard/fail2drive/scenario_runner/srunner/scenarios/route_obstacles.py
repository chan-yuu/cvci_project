...
class ParkedObstacle(BasicScenario):
    ...
    def _spawn_obstacle(self, wp, blueprint):
        ...
        spawn_transform = wp.transform
        spawn_transform.location += carla.Location(x=displacement * r_vec.x, y=displacement * r_vec.y, z=1)
        self.parking_slots.append(spawn_transform.location)
        actor = CarlaDataProvider.request_new_actor(
            blueprint, spawn_transform, rolename='scenario no lights', attribute_filter={'base_type': 'car', 'generation': 2},
        )
        if not actor:
...
class BadParkingObstacle(ParkedObstacle):
    def _initialize_actors(self, config):
        """
        Custom initialization
        """
        ...
        blueprint = get_value_parameter(config, 'vehicle', str, "vehicle.*")
        x = get_value_parameter(config, 'x', float, 0)
        y = get_value_parameter(config, 'y', float, 1.5)
        yaw = get_value_parameter(config, 'yaw', float, 0)

        parking_tf = self._vehicle_wp.transform
        parking_tf.location += parking_tf.rotation.get_forward_vector() * x
        parking_tf.location += parking_tf.rotation.get_right_vector() * y

        self.parking_slots.append(parking_tf.location)

        parking_tf.location.z += 1
        parking_tf.rotation.yaw += yaw

        parked_actor = CarlaDataProvider.request_new_actor(blueprint, parking_tf)
        if not parked_actor:
            raise ValueError("Couldn't spawn an obstacle actor")
...
# This is just a copy of the code above:
class BadParkingObstacleTwoWays(BadParkingObstacle):
    """
    Variation of the ParkedObstacle scenario but the ego now has to invade the opposite lane
    """
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True, timeout=180):
