...
class InfractionRecorder:
    ...
    def __init__(
        ...
        Args:
            config_evaluation: Evaluation config used for output settings.
    ...
    def _persist_to_json(self) -> None:
        ...
        if self.config_evaluation.save_path is None:
            return

        infractions_path = self.config_evaluation.save_path / "infractions.json"
        infractions_data: dict[str, Any] = {
            "infractions": self.infractions_log,
            "video_fps": self.config_evaluation.video_fps,
        }
