...
class PrivilegedRoutePlanner:
    ...
    def __init__(self, config: ExpertConfig):
        ...
        self.points_per_meter = self.config.simulation.points_per_meter
        self.ego_vehicles_route_point_search_distance = (
            self.config.driving.ego_vehicles_route_point_search_distance
        )
        self.lane_shift_extension_length_for_yield_to_emergency_vehicle = self.config.driving.lane_shift_extension_length_for_yield_to_emergency_vehicle
        self.transition_smoothness_distance = (
            self.config.driving.transition_smoothness_distance
        )
        self.route_shift_start_distance_invading_turn = (
            self.config.driving.route_shift_start_distance_invading_turn
        )
        self.route_shift_end_distance_invading_turn = (
            self.config.driving.route_shift_end_distance_invading_turn
        )
        self.fence_avoidance_margin_invading_turn = (
            self.config.driving.fence_avoidance_margin_invading_turn
        )
        self.minimum_lane_width_threshold = (
            self.config.driving.minimum_lane_width_threshold
        )
        self.leading_vehicles_max_route_distance = (
            self.config.driving.leading_vehicles_max_route_distance
        )
        self.leading_vehicles_max_route_angle_distance = (
            self.config.driving.leading_vehicles_max_route_angle_distance
        )
        self.leading_vehicles_maximum_detection_radius = (
            self.config.driving.leading_vehicles_maximum_detection_radius
        )
        self.trailing_vehicles_max_route_distance = (
            self.config.driving.trailing_vehicles_max_route_distance
        )
        self.trailing_vehicles_max_route_distance_lane_change = (
            self.config.driving.trailing_vehicles_max_route_distance_lane_change
        )
        self.tailing_vehicles_maximum_detection_radius = (
            self.config.driving.tailing_vehicles_maximum_detection_radius
        )
        self.max_distance_lane_change_trailing_vehicles = (
            self.config.driving.max_distance_lane_change_trailing_vehicles
        )
        self.extra_route_length = self.config.driving.extra_route_length
    ...
    def extend_lane_shift_transition_for_yield_to_emergency_vehicle(
        ...
        for idx in range(transition_start_index, transition_end_index):
            ...
            target_lane_location = np.array(
                [
                    target_lane_location.x,
                    target_lane_location.y,
                    target_lane_location.z,
                ],
            )
    ...
    def extend_lane_shift_transition_for_hazard_at_side_lane(
        ...
        obstacle_route_index = self.get_closest_route_index(
            int(self.route_index),
            last_bicycle.get_location(),
        )
    ...
    def setup_route(
        ...
        # When the route starts with a ParkingExit scenario, the first waypoint is on
        # the road center, not in the parking lot where the agent starts.
    ...
    def smooth_and_supersample(
        ...
        segment_indices = np.insert(
            np.argwhere(cumulative_distances[1:] < cumulative_distances[:-1]),
            0,
            0,
        )
    ...
    def compute_distances_to_stop_signs(
        ...
        The detection matches RunningStopTest in
        scenario_runner/srunner/scenariomanager/scenarioatomics/atomic_criteria, the
        criterion that judges whether a stop sign was run.
        ...
        def is_actor_affected_by_stop(wp_list, stop_extent, stop_location):
            ...
            # More than one waypoint avoids small trigger volumes and backward movement.
    ...
    def compute_leading_vehicles(
        ...
        if len(vehicle_ids) and self.route_index != self.route_points.shape[0]:
            ...
            # A vehicle is leading if it lies close to the route ahead and its
            # orientation roughly follows the road; both checks together exclude
            # vehicles that merely cross the ego vehicle's future path.
            distances = (
    ...
    def compute_trailing_vehicles(
        ...
        for i in range(
            max(0, self.route_index - max_distance_lane_change),
            self.route_index,
        ):
