...
class TransfuserDataset(AbstractPolicyDataset):
    ...
    def cache_finger_print(self) -> dict[str, str]:
        ...
        finger_print["use_lidars"] = str(self.lead_config.expert.sensor_rig.use_lidars)
        finger_print["use_hd_map"] = str(self.lead_config.expert.sensor_rig.use_hd_map)
        return finger_print
    ...
    def get_sample_parts(self) -> dict[str, SamplePart]:
        ...
        use_lidars = lead_config.expert.sensor_rig.use_lidars
        use_hd_map = lead_config.expert.sensor_rig.use_hd_map
        anchor_only = (0,)
        ...
        if not config.LTF and use_lidars:
        ...
        elif not config.LTF and use_hd_map:
            parts["hd_map_raster"] = SamplePart(
                reads=SceneLoadingSpec(read_map_api=True),
                builds=partial(build_hd_map_raster, lead_config=lead_config),
                # Values are 0.35 / 0.70 / 0.85 / 1.0; scale 100 keeps them distinct.
                caches={
                    "rasterized_lidar": {
                        "name": "png",
