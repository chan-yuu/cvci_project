...
class AbstractDrivingAgent(BaseAgent, autonomous_agent.AutonomousAgent, abc.ABC):
    ...
    def setup(  # pyright: ignore[reportIncompatibleMethodOverride]
        ...
        if lead_config.expert.sensor_rig.use_lidars:
        ...
        if lead_config.expert.sensor_rig.use_radars:
    ...
    def tick(self, sensor_data: CarlaSensorData) -> CarlaSensorData:
        ...
        if (
            self.lidar_sweep_queue.maxlen
            and self.lead_config.expert.sensor_rig.use_lidars
        ):
    ...
    def build_scene_data(self, sensor_data: CarlaSensorData) -> SceneData:
        ...
        return SceneData(
            ...
            lidar_sweeps=(
                dict(enumerate(reversed(self.lidar_sweep_queue)))
                if self.lead_config.expert.sensor_rig.use_lidars
                else None
            ),
            radar_sweeps=(
    ...
    def _ego_state(self, sensor_data: CarlaSensorData) -> EgoStateSE3:
        """The ego state the sensors provide: GNSS+IMU pose when HD-map is on.

        The simulator's true actor pose stays privileged. When the BEV branch
        reads a local map tile, the pose is the same GNSS+IMU localization a
        real vehicle uses to query a map SDK. Otherwise the pose is identity
        and only the speedometer velocity is consumed.
        """
        if (
            self.lead_config.expert.sensor_rig.use_hd_map
            and self.compass is not None
        ):
            position = self.localized_position
            center_se3 = carla_to_123d.carla_planar_pose_to_iso_se3(
                float(position[0]),
                float(position[1]),
                float(self.compass),
            )
    ...
    def _init(self) -> None:
        ...
        if self.lead_config.expert.sensor_rig.use_hd_map:
            self._map_api = self._load_town_map_api()

        self.initialized = True

    def _load_town_map_api(self) -> typing.Any:
        """Town OpenDRIVE as a 123D map, the same tile a vehicle map SDK serves."""
        town_name = self._world.get_map().name.split("/")[-1]
        location = town_name.lower()
        dataset = self.lead_config.expert.data_collection.py123d_dataset
        map_path = (
            Path(get_dataset_paths().py123d_maps_root)
            / dataset
            / f"{dataset}_{location}.arrow"
        )
        if not map_path.exists():
            LOG.warning(
                "HD-map arrow missing at %s; BEV map will be route-only until "
                "the town is converted during collection",
                map_path,
            )
            return None
        return get_lru_cached_map_api(map_path)

    def sensors(self) -> list[SensorSpec]:
        return av_sensor_setup(
            config=self.lead_config.expert,
            lidar=self.lead_config.expert.sensor_rig.use_lidars,
